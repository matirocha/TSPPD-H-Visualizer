...
def generate_step_explanation(step_idx, total_steps, from_node, to_node, dist, deliver_a, pickup_b, a_on_truck, b_on_truck, handling_count, handling_cost, h_val, re_b_count=0, re_a_count=0):
    ...
    if step_idx == 0:
        ...
        if handling_count > 0:
            motives = []
            if re_b_count > 0:
                motives.append(f"{re_b_count} por obstrucción β en puerta")
            if re_a_count > 0:
                motives.append(f"{re_a_count} α reubicadas proactivamente al frente")
            motive_str = f" ({', '.join(motives)})" if motives else ""
            handling_txt = (
                f" ⚠️ <strong>Handling LIFO:</strong> Se requirieron {handling_count} operaciones de manipulación{motive_str}. "
...
def print_detailed_schedule(tour, c, alpha, beta, Q, positions, h_val, steps, total_dist, total_handling_cost, obj_val, instance_id, num_customers):
    ...
    for step in steps[:-1]:
        ...
        if num_re > 0:
            print(f"   • Manipulación y Descarga temporal (Handling):")
            print(f"     - Profundidad alcanzada en el compartimiento: hasta el slot {k_max} (de {Q}).")
            if len(re_b) > 0:
                print(f"     - Obstrucción física en puerta (LIFO):")
                print(f"       * {len(re_b)} unidades tipo β que bloqueaban el acceso a las entregas (Slots: {format_slot_ranges(re_b)})")
            if len(re_a) > 0:
                print(f"     - Reubicación proactiva (optimización para clientes posteriores):")
                print(f"       * {len(re_a)} unidades tipo α de otros clientes (Slots: {format_slot_ranges(re_a)}) extraídas para reubicarse al frente y evitar handling futuro")
        else:
    ...
    if m.Status == GRB.OPTIMAL:
        ...
        for s in range(total_steps):
            ...
            if v_node != 0:
                ...
                # Identificar candidatos de mercancía α descargada (aquellas que no siguen en la misma posición)
                cand_a_unloaded = [k for k in items_acc_a if k not in alpha_out_slots]
                
                # Regla LIFO: Los primeros slots de α hacia la compuerta se entregan al cliente
                delivered_a_slots = cand_a_unloaded[:del_a]
                
                # Los slots α restantes descargados para reubicación o acceso al fondo
                rehandled_a = cand_a_unloaded[del_a:] + [k for k in items_acc_a if k in alpha_out_slots]
                
                h_count = len(rehandled_b) + len(rehandled_a)
                h_cost = round(len(rehandled_b) * h_b + len(rehandled_a) * h_a, 4)
                
                # Slots de recolección β nueva asignados en esta parada
                cand_b_new = [k for k in beta_out_slots if k not in beta_in_slots]
                new_b_slots = cand_b_new[:pick_b] if pick_b > 0 else []
            ...
            explanation = generate_step_explanation(
                ...
                h_val=h_val,
                re_b_count=len(rehandled_b),
                re_a_count=len(rehandled_a)
            )
