...
app.get('/api/solutions', (req, res) => {
  try {
    ...
    res.json(files)
  } catch (err) { res.status(500).json({ error: 'Failed to read solutions' }) }
})
...
app.get('/api/solutions/:filename', (req,res)=>{
  try{
    ...
    res.send(content)
  } catch(err){ res.status(500).json({error:'Failed to read'}) }
})
...
app.listen(PORT, ()=>{
  console.log('====================================================')
  console.log('  TSPPD-H Visualizador · Pagina Web 7 (Muse Park 1.2)')
